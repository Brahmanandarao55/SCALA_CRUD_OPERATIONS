...
class UnitTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  val filepath = "C:\\Users\\Brahmananda Rao\\Desktop\\data.csv"
  val person: Person = Person("ravi", 22, 45000, "Developer", "Hyderabad")
  ...
  it should "update a Person" in{
    service.update(person,2)
    verify(repo).updatePerson(person,2)
